...
// extern SPI_HandleTypeDef st_handle;
//
...
void ST7789_WriteCommand(uint8_t cmd) {
  ST7789_CS_Set(1);
  ST7789_DC_Set(0);
  HAL_SPI_Transmit(&st_handle, &cmd, sizeof(cmd), HAL_MAX_DELAY);
  ST7789_CS_Set(0);
}
...
void ST7789_WriteData(uint8_t *buf, uint16_t buf_size) {
  ...
  while (buf_size > 0) {
    ...
    HAL_SPI_Transmit(&st_handle, buf, chunk_size, HAL_MAX_DELAY);
    buf += chunk_size;
    buf_size -= chunk_size;
  }
...
void ST7789_WriteSmallData(uint8_t data) {
  ST7789_CS_Set(1);
  ST7789_DC_Set(1);
  HAL_SPI_Transmit(&st_handle, &data, sizeof(data), HAL_MAX_DELAY);
  ST7789_CS_Set(0);
}
